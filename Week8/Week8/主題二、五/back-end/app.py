...
def signup():
    ...
    if name.strip() == "" or username.strip() == "" or password.strip() == "":
        response = {"type": "fail", "message": "註冊欄位不得為空白"}
        return jsonify(response)

    # 驗證格式
    if not is_valid_name(name):
        response = {"type": "fail", "message": "姓名格式不正確，請輸入中文或英文，至少兩個字元"}
        return jsonify(response)

    if not is_valid_username(username):
        response = {"type": "fail", "message": "帳號格式不正確，請輸入英文或數字，至少六個字元"}
        return jsonify(response)

    if not is_valid_password(password):
        response = {"type": "fail",
                    "message": "密碼格式不正確，請輸入包含一個大寫英文、一個小寫英文以及數字，至少六個字元"}
        return jsonify(response)
...
def signin():
    ...
    if username.strip() == "" or password.strip() == "":
        response = {"type": "fail", "message": "登入欄位不得為空白"}
        return jsonify(response)
...
def api_get_member():
    ...
    # 判斷是否為空白欄位
    if username.strip() == "":
        response = {"type": "fail", "message": "帳號不得為空白"}
        return jsonify(response)
...
def api_update_member_name():
    ...
    # 判斷是否為空白欄位
    if new_name.strip() == "":
        response = {"type": "fail", "message": "姓名不得為空白"}
        return jsonify(response)
...
def create_message():
    ...
    # 判斷是否為空白欄位
    if content.strip() == "":
        response = {"type": "fail", "message": "留言不得為空白"}
        return jsonify(response)
